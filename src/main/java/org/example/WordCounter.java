...
public class WordCounter {
    ...
    // calculateWord metodunu tanımlıyoruz
    public static Map<String, Integer> calculatedWord() {
        ...
        // Kelimeleri içeren bir Map oluşturuyoruz
        Map<String, Integer> wordCount = new HashMap<>();

        // Metni kelimelere ayırıyoruz. Her kelimeyi küçük harf yapmak, noktalama işaretlerini temizlemek için regex kullanıyoruz.
        String[] words = text.toLowerCase().replaceAll("[^a-zA-Z ]", "").split("\\s+");

        // Her kelimeyi tek tek sayıyoruz
        for (String word : words) {
            // Eğer kelime mapte mevcutsa, sayısını artırıyoruz. Değilse, 1 olarak başlatıyoruz.
